...
// Classement pour un adversaire donné (calendrier fixe des matchs amicaux),
// indépendamment du format exact de la date dans le fichier d'export.
export function rankByOpponent(rows, opponentName) {
  if (!opponentName) return [];
  const target = opponentName.trim().toLowerCase();
  const filtered = rows.filter((r) => r.adversaire && r.adversaire.trim().toLowerCase() === target);
  return sortDesc(filtered.map((r) => ({ pseudo: r.pseudo, total: r.score })));
}
