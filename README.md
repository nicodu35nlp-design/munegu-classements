...
## La source de données doit être la même pour tout le monde

Important : pour que tous les joueurs voient les **mêmes** classements
(et pas chacun sa propre copie locale), le lien du Google Sheet publié doit
être défini **une fois pour toutes au déploiement**, via une variable
d'environnement — pas juste tapé dans le champ de `/backstage` sur un seul
ordinateur (ce champ ne sert qu'à tester un lien différent en local, il ne
se propage pas aux autres visiteurs).

Dans les paramètres du projet sur Vercel/Netlify, ajouter :
...
En local, créer un fichier `.env` à la racine avec les mêmes variables (déjà
